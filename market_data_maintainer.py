...
def _download_batch(tickers, start_date, field="Close", retries=DOWNLOAD_RETRIES, use_threads=False):
    clean = [t.replace("/", "-") for t in tickers]
    result = pd.DataFrame()

    for attempt in range(retries):
        try:
            df = _download_raw(clean, start_date, threads=bool(use_threads))
            if df is None or df.empty:
