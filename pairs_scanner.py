...
# Improved: half-life of mean reversion is now a major factor
W_ZSCORE   = 0.30   # Z-score magnitude — how extreme the current spread is
W_CORR     = 0.15   # Long-term correlation quality — are these genuinely co-moving?
W_CORR_BRK = 0.15   # Correlation breakdown — pair is currently diverging from norm
W_HALFLIFE = 0.25   # OU half-life speed — faster reverting spreads are better trades
W_REL_PERF = 0.15   # Relative performance — confirms the divergence is real
...
def split_save(df, prefix):
    etfs = [c for c in df.columns if TICKER_TYPES.get(c, "Pure Stock") == "Pure ETF"]
    stocks = [c for c in df.columns if TICKER_TYPES.get(c, "Pure Stock") == "Pure Stock"]
    
    if etfs:
        tmp = f"{prefix}_etfs.csv.gz.tmp"
        df[etfs].to_csv(tmp, compression='gzip')
        os.replace(tmp, f"{prefix}_etfs.csv.gz")
        
    if stocks:
        tmp = f"{prefix}_stocks.csv.gz.tmp"
        df[stocks].to_csv(tmp, compression='gzip')
        os.replace(tmp, f"{prefix}_stocks.csv.gz")


# ==========================================
# LOAD CACHE
# ==========================================
def split_load(prefix):
    dfs = []
    for suffix in ["_etfs.csv.gz", "_stocks.csv.gz"]:
        path = f"{prefix}{suffix}"
        if os.path.exists(path):
            try: dfs.append(pd.read_csv(path, index_col=0, parse_dates=True))
            except: pass
...
def get_cache_age(prefix):
    path = f"{prefix}_etfs.csv.gz"
    if not os.path.exists(path):
        path = f"{prefix}_stocks.csv.gz"
    if os.path.exists(path):
        return (datetime.now() - datetime.fromtimestamp(os.path.getmtime(path))).total_seconds() / 3600
    return 9999
...
def build_dataset(master):
    data = split_load("historical_data")

    if not data.empty and get_cache_age("historical_data") < CACHE_UPDATE_COOLDOWN_HOURS:
        print(f"--- Cache is fresh. Skipping download. ---")
        return data[[c for c in data.columns if c in master]]
    ...
    if missing:
        ...
        for i, idx in enumerate(range(0, len(missing), BATCH_SIZE)):
            ...
            if not batch_df.empty:
                ...
                split_save(data, "historical_data")
            time.sleep(COOLDOWN)
...
def build_chart_dataset(master):
    ...
    chart_data = split_load("chart_data")
    if not chart_data.empty and get_cache_age("chart_data") < CACHE_UPDATE_COOLDOWN_HOURS:
        print(f"--- Chart cache fresh. Skipping chart download. ---")
        return chart_data[[c for c in chart_data.columns if c in master]]
    ...
    if missing:
        ...
        for i, idx in enumerate(range(0, len(missing), BATCH_SIZE)):
            ...
            if not batch_df.empty:
                ...
                split_save(chart_data, "chart_data")
            time.sleep(COOLDOWN)
...
def build_volume_dataset(master):
    ...
    all_vol = split_load("volume_data")
    if not all_vol.empty and get_cache_age("volume_data") < CACHE_UPDATE_COOLDOWN_HOURS:
        print(f"--- Volume cache fresh. ---")
        for col in all_vol.columns:
            series = all_vol[col].dropna()
            if len(series) > 0: vol_avg[col] = float(series.rolling(VOL_AVG_DAYS, min_periods=1).mean().iloc[-1])
        return vol_avg
    ...
    for i, idx in enumerate(tqdm(range(0, len(master), BATCH_SIZE), desc="Downloading Volume")):
    ...
    if not all_vol.empty:
        ...
        split_save(all_vol, "volume_data")
        for col in all_vol.columns:
            series = all_vol[col].dropna()
            if len(series) > 0:
...
def analyze_pair(pair):
    ...
    # ── Long-term correlation filter ────────────────────────────────────────
    cl = corr_long.loc[a, b]
    if cl < MIN_CORR_FILTER:
        return None

    cs = corr_short.loc[a, b]
    # corr_brk > 0: short-term correlation has dropped below long-term mean
    # — the pair is actively diverging, supporting a mean-reversion entry
    corr_brk = cl - cs

    # ── Spread & Z-score ────────────────────────────────────────────────────
    ratio = log_prices[a] - log_prices[b]   # log(A/B): the spread
    mean  = ratio.mean()
    std   = ratio.std()
    if std == 0:
        return None

    z  = (ratio.iloc[-1] - mean) / std      # standard deviations from mean
    rp = perf[a] - perf[b]                  # relative performance (% diff)

    # ── Ornstein-Uhlenbeck half-life ────────────────────────────────────────
    # Fit AR(1) to the spread: Δspread = β * spread(t-1) + α + ε
    # β < 0 → mean reverting; half-life = -ln(2) / β (continuous-time approx)
    spread_full = all_returns_for_hl[a] - all_returns_for_hl[b]
    spread_full = spread_full.dropna()
    half_life   = np.inf
    if len(spread_full) >= 30:
        try:
            s_lag  = spread_full.shift(1).dropna()
            s_diff = spread_full.diff().dropna()
            idx    = s_lag.index.intersection(s_diff.index)
            if len(idx) >= 20:
                lag_vals = s_lag.loc[idx].values
                diff_vals = s_diff.loc[idx].values
    
                # If the spread has basically zero variation, it will never mean-revert
                if np.std(lag_vals) < 1e-8:
                    return 99999
        
                # Ignore the warning if it still triggers on borderline cases
                import warnings
                with warnings.catch_warnings():
                warnings.simplefilter('ignore', np.RankWarning)
                beta = np.polyfit(lag_vals, diff_vals, 1)[0]  
                if beta < 0:
                    half_life = min(-np.log(2) / beta, 500)   # cap at 500 days
        except Exception:
            pass

    # ── Estimated gross return if spread fully reverts ──────────────────────
    # Spread return = |Z| * σ_spread (in log-return units ≈ % for small values)
    est_ret_pct = abs(z) * std * 100        # percentage
    # Annualised: assume we get this trade once per half-life cycle
    if half_life < 500:
        ann_ret_pct = round(est_ret_pct * (252.0 / max(half_life, 5)), 1)
    else:
        ann_ret_pct = None                  # spread does not mean-revert reliably

    # ── Normalised scoring components ───────────────────────────────────────
    # z_norm: |Z| = 3 → full score. Rewards extreme spread departures.
    z_norm = min(abs(z) / 3.0, 1.0)

    # corr_quality: long-term correlation quality (0.6 baseline → 0 score, 1.0 → full)
    corr_qual_norm = min(max(cl - MIN_CORR_FILTER, 0) / (1.0 - MIN_CORR_FILTER), 1.0)

    # corr_brk_norm: a drop of 0.5 in correlation → full score
    corr_brk_norm = min(max(corr_brk, 0) / 0.5, 1.0)

    # hl_norm: half-life ≤ 5 days → 1.0; ≥ 100 days → 0.0
    # Rewards pairs that historically snap back quickly
    hl_norm = max(0.0, 1.0 - (half_life / 100.0)) if half_life < 500 else 0.0

    # perf_norm: 10% performance gap → full score
    perf_norm = min(abs(rp) / 10.0, 1.0)

    score = (
        W_ZSCORE   * z_norm
        + W_CORR     * corr_qual_norm
        + W_CORR_BRK * corr_brk_norm
        + W_HALFLIFE * hl_norm
        + W_REL_PERF * perf_norm
    )
    ...
    hl_display = round(half_life, 1) if half_life < 500 else None
...
def build_symbols_page(valid_tickers):
    ...
    df_etf   = read_csv_meta("ETFs.csv")
    df_stock = read_csv_meta("STOCKS.csv")
...
if __name__ == "__main__":
    ...
    # Full log-price series for OU half-life regression (use all available history)
    all_returns_for_hl = np.log(data)
    ...
    for i, r in enumerate(top_results):
        ...
        <tr class="data-row" data-category="{r['Category']}" data-z="{z}"
          ...
          <td class="est-cell">
            ...
            {f'<span class="ann-ret">{ann_ret:+.0f}%/yr</span>' if ann_ret else ''}
          </td>
          <td class="score-cell">
...
<style>
  ...
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .pair-fullnames {{ display: flex; flex-direction: column; gap: 1px; margin-top: 3px; }}
  .name-a  {{ font-size: 12px; color: #4a8aaa; white-space: normal; line-height: 1.4; }}
  .name-b  {{ font-size: 12px; color: #6b7f9a; white-space: normal; line-height: 1.4; }}
  ...
  .hl-value {{ font-family: var(--mono); font-size: 13px; font-weight: 600; color: #a78bfa; }}
  .hl-na    {{ color: var(--muted); font-size: 12px; }}
  ...
  /* Keep tooltip on-screen for first/last columns */
  th:nth-child(2).has-tip::after {{ left: 0; transform: none; }}
  th:nth-child(2).has-tip:hover::after {{ transform: none; }}
  th:last-child.has-tip::after, th:nth-last-child(2).has-tip::after {{
    left: auto; right: 0; transform: none;
  }}
  th:last-child.has-tip:hover::after, th:nth-last-child(2).has-tip:hover::after {{ transform: none; }}
...
<tr>
  <th>#</th>
  <th>Pair / Name</th>
  <th class="has-tip" onclick="setSort('z_abs')" data-tip="Z-Score measures how many standard deviations the current price spread (log ratio) is from its {Z_LENGTH}-day rolling mean. Positive Z = Leg A is overpriced vs B. Negative = A is underpriced. Higher |Z| = more extreme divergence and a stronger mean-reversion opportunity.">Z-Score &#8597; &#9432;</th>
  <th class="has-tip" onclick="setSort('corr')" data-tip="Corr: the {CORR_LONG}-day Pearson correlation of daily returns — how reliably these two instruments move together over time (must be &ge; {MIN_CORR_FILTER} to qualify). &Delta;: the correlation breakdown = long-term corr minus {CORR_SHORT}-day short-term corr. A positive delta means the pair is currently diverging from its historical co-movement, supporting a mean-reversion entry.">Corr / &Delta; &#8597; &#9432;</th>
  <th class="has-tip" onclick="setSort('hl')" data-tip="Half-Life: the estimated number of days for the spread to travel halfway back to its mean, derived from an Ornstein-Uhlenbeck AR(1) regression on the spread. Shorter = faster mean reversion. Below 20 days is excellent; above 60 days is slow and risky.">Half-Life &#8597; &#9432;</th>
  <th class="has-tip" onclick="setSort('est_ret')" data-tip="Est Return: the gross spread return if the spread fully reverts from its current Z-score back to the mean (= |Z| &times; &sigma;_spread). This is the theoretical maximum on the spread, not per-leg. Ann Return: extrapolates this assuming the trade repeats once per half-life cycle over a year — a rough upper bound for position sizing context.">Est Return &#8597; &#9432;</th>
  <th class="has-tip" onclick="setSort('perf')" data-tip="Performance Difference: the return of Leg A minus the return of Leg B over the past {PERF_LENGTH} days (%). Positive means A has outperformed B recently. When combined with a high Z-score, it confirms the divergence is real price action rather than just volatility noise.">Perf Diff &#8597; &#9432;</th>
  <th class="has-tip" onclick="setSort('score')" data-tip="Composite Score (0–1): weighted combination of 5 factors: {int(W_ZSCORE*100)}% Z-Score magnitude + {int(W_CORR*100)}% correlation quality + {int(W_CORR_BRK*100)}% correlation breakdown + {int(W_HALFLIFE*100)}% mean-reversion speed (half-life) + {int(W_REL_PERF*100)}% relative performance. Higher = stronger overall trade setup.">Score &#8597; &#9432;</th>
  <th class="has-tip" data-tip="Trade Signal: the recommended direction based on the Z-score. Positive Z → Leg A is rich relative to B → SHORT A / LONG B. Negative Z → A is cheap → LONG A / SHORT B. Strong signals exceed the {Z_STRONG}&sigma; threshold; standard signals exceed {Z_THRESHOLD}&sigma;.">Signal &#9432;</th>
  <th style="text-align:center;">Z-Chart</th>
  <th class="has-tip" style="text-align:right;" data-tip="Shares to buy or sell for Leg A based on the capital you entered above, split 50/50 between legs. Share count = (capital / 2) / last close price. Price shown below is the last close. Avg volume shown is the {VOL_AVG_DAYS}-day rolling average daily volume.">Leg A — Shares &#9432;</th>
  <th class="has-tip" style="text-align:right;" data-tip="Shares to buy or sell for Leg B. Sized to dollar-match Leg A so the pair is capital-neutral. Price and avg volume shown below share count.">Leg B — Shares &#9432;</th>
</tr>
</thead>
<tbody id="tableBody">
{rows_html}
...
<div class="footer">
  ...
  <div>
    Score = {int(W_ZSCORE*100)}% |Z| + {int(W_CORR*100)}% Corr Quality + {int(W_CORR_BRK*100)}% Corr Break + {int(W_HALFLIFE*100)}% Half-Life + {int(W_REL_PERF*100)}% Perf
...
function openChart(btn) {{
  ...
  const {{ pair, nameA, nameB, dates, z, zWindow, currentZ, halfLife, estRet, annRet }} = d;
  ...
  const hlStr  = halfLife  ? halfLife.toFixed(0) + "d"  : "—";
  const estStr = estRet   ? "+" + estRet.toFixed(1) + "%" : "—";
  const annStr = annRet   ? annRet.toFixed(0) + "%/yr"    : "—";
  document.getElementById("modalStats").innerHTML = `
    ...
    <div class="mstat">
      ...
      <div class="mstat-value" style="color:#4a6080;">${{dates.length}}d</div>
    </div>`;
...
function sortTable() {{
  ...
  const numOf = (row, sel) => parseFloat(row.querySelector(sel)?.textContent?.replace(/[^0-9.+-]/g,"")) || 0;
  rows.sort((a, b) => {{
    if (key === "score")   return numOf(b,".score-num") - numOf(a,".score-num");
    if (key === "z_abs")   return Math.abs(parseFloat(b.dataset.z)) - Math.abs(parseFloat(a.dataset.z));
    if (key === "corr")    return numOf(b,".corr-value") - numOf(a,".corr-value");
    if (key === "perf")    return Math.abs(numOf(b,".perf-cell span")) - Math.abs(numOf(a,".perf-cell span"));
    if (key === "hl") {{
      // sort ascending (shorter half-life = better), put "—" last
      const ha = numOf(a,".hl-value") || 99999;
      const hb = numOf(b,".hl-value") || 99999;
      return ha - hb;
    }}
    if (key === "est_ret") return numOf(b,".est-ret") - numOf(a,".est-ret");
    return 0;
  }});
