...
# ── Scoring Weights ──
W_BACKTEST   = 0.35                            # Backtest win rate
# Pre-backtest weights (must sum to 1.0 for correct normalization)
W_ZSCORE     = 0.15                            # Z-score magnitude (how far from mean)
W_HALFLIFE   = 0.15                            # Half-life speed (faster reversion = better)
W_CONFIRM    = 0.10                            # Timeframe confirmation (alignment + confidence)
...
_INVALID_TICKERS = {"NASDAQ", "NYS", "NYSE", "NYSEARCA", "ARCA", "AMEX"}

def load_master_tickers():
    global TICKER_TYPES, TICKER_NAMES, TICKER_INDUSTRY, TICKER_SUBIND, TICKER_SUBIND2
    global TICKER_CSV_MCAP, TICKER_CSV_VOL, ETF_LEV_TYPES
    tickers = []
    ...
    if os.path.exists("ETFs.csv"):
        ...
        # ETFs.csv format (matches stock_dashboard):
        # 0 Ticker, 1 Name, 2 Type, 3 Sector, 4 Industry, 5 MCap, 6 Volume
        for _, row in df_etf.iterrows():
            t = str(row.iloc[0]).strip().upper()
            if (not t) or t in ("", "NONE", "NAN", "SYMBOL", "TICKER") or t in _INVALID_TICKERS or t.startswith("$"):
                continue
            ...
            if ncols >= 3 and pd.notna(row.iloc[2]):
                raw_type = str(row.iloc[2]).strip().lower()
                ETF_LEV_TYPES[t] = _ETF_TYPE_MAP.get(raw_type, "normal")
            else:
                ETF_LEV_TYPES[t] = "normal"
            if ncols >= 4 and pd.notna(row.iloc[3]):
                TICKER_INDUSTRY[t] = str(row.iloc[3]).strip()   # sector (ETF)
            if ncols >= 5 and pd.notna(row.iloc[4]):
                TICKER_SUBIND[t] = str(row.iloc[4]).strip()     # industry (ETF)
            if ncols >= 6 and pd.notna(row.iloc[5]):
                TICKER_CSV_MCAP[t] = _parse_mcap_str(row.iloc[5])  # AUM/asset value
            if ncols >= 7 and pd.notna(row.iloc[6]):
                TICKER_CSV_VOL[t] = _parse_vol_str(row.iloc[6])   # avg 10-day volume
    ...
    if os.path.exists("STOCKS.csv"):
        ...
        for _, row in df_stock.iterrows():
            ...
            if ncols >= 7 and pd.notna(row.iloc[6]):
                TICKER_CSV_MCAP[t] = _parse_mcap_str(row.iloc[6])
            if ncols >= 8 and pd.notna(row.iloc[7]):
                TICKER_CSV_VOL[t] = _parse_vol_str(row.iloc[7])

    tickers = list(set(tickers))
    tickers = [t for t in tickers if t not in ["", "NONE", "NAN", "SYMBOL", "TICKER"]
               and t not in _INVALID_TICKERS and not t.startswith("$")]
...
# LOAD CACHE
# ==========================================
def load_cache(path, label=None):
    if os.path.exists(path):
        try:
            df = pd.read_csv(path, index_col=0, parse_dates=True)
            df = df.loc[:, ~df.columns.duplicated()]
            if label:
                print(f"{label} cache loaded.")
            return df
        except Exception:
            pass
    return pd.DataFrame()
...
def load_combined_cache():
    stock = load_cache(STOCK_DATA_FILE, label="Stock")
    etf   = load_cache(ETF_DATA_FILE, label="ETF")
    ...
    if etf.empty:
        return stock
    data = pd.concat([stock, etf], axis=1)
...
def build_volume_dataset(master):
    """Returns a dict {ticker: avg_volume} using CSV-provided avg 10-day volume."""
    vol_avg = {}
    missing = 0
    for t in master:
        v = TICKER_CSV_VOL.get(t, 0)
        if v and v > 0:
            vol_avg[t] = int(v)
        else:
            missing += 1
    print(f"  Volume (CSV avg 10d): {len(vol_avg)} tickers ready. Missing: {missing}")
    return vol_avg


def build_market_cap(master):
    """Returns a dict {ticker: market_cap_or_assets} from CSV (no yfinance)."""
    mcap = {}
    missing = 0
    for t in master:
        v = TICKER_CSV_MCAP.get(t, 0)
        if v and v > 0:
            mcap[t] = int(v)
        else:
            missing += 1
    print(f"  Market cap / assets (CSV): {len(mcap)} tickers ready. Missing: {missing}")
    return mcap
...
def analyze_pair(pair):
    ...
    # Z-score on last Z_LENGTH days using hedge-ratio-weighted spread (fixed window)
    ratio = log_prices[a] - hedge_ratio * log_prices[b]
    mean  = ratio.mean()
    std   = ratio.std()
    ...
    spread_std = std  # for SpreadStd (used in trade tracker)

    # Raw spread std for estimated return — hedge-ratio spread has minimized variance,
    # but actual dollar P&L scales with the raw price divergence
    raw_ratio = log_prices[a] - log_prices[b]
    raw_std = raw_ratio.std()
    est_spread_std = raw_std if raw_std > 0 else std
    ...
    # ── Adaptive Z-score window based on half-life (fixed, hedge-ratio weighted) ──
    adaptive_window = int(max(50, min(250, hl * 5)))
    lp_full = log_prices_full
    ratio_adapt = lp_full[a].iloc[-adaptive_window:] - hedge_ratio * lp_full[b].iloc[-adaptive_window:]
    ...
    # ── Multi-timeframe Z-scores (30d short, 250d long, fixed window, hedge-ratio weighted) ──
    ratio_s = log_prices_short[a] - hedge_ratio * log_prices_short[b]
    std_s   = ratio_s.std()
    z30     = round((ratio_s.iloc[-1] - ratio_s.mean()) / std_s, 2) if std_s > 0 else 0.0

    ratio_l = log_prices_long[a] - hedge_ratio * log_prices_long[b]
    std_l   = ratio_l.std()
    ...
    est_ret = round(abs(z) * est_spread_std * 100, 2)   # in %
...
# ==========================================
def compute_z_history(a, b, price_data, window_override=None, hedge_ratio=1.0):
    """Fixed-window Z-score history using hedge-ratio-weighted spread.
    Computes the spread mean and std over a single fixed lookback window,
    then applies those fixed stats to every point in the series.
    This ensures Z=0 always refers to the same spread level, so
    reversion on the chart corresponds to actual P&L.
    If window_override is given, uses that as the fixed window length.
    """
    log_a = np.log(price_data[a].dropna().clip(lower=1e-10))
    log_b = np.log(price_data[b].dropna().clip(lower=1e-10))
    ...
    # Fixed stats: compute mean/std over the last `window` days
    ref_spread = spread.iloc[-window:]
    fixed_mean = ref_spread.mean()
    fixed_std  = ref_spread.std()
    if fixed_std == 0:
        return [], []

    # Apply fixed mean/std to the full history (so chart shows Z relative to one baseline)
    z_series = (spread - fixed_mean) / fixed_std
...
def build_symbols_page(valid_tickers, dropped_tickers=None):
    ...
    # ── CSV column layouts ────────────────────────────────────────────────────
    # ETFs.csv:   Ticker, Name, Type, Sector, Industry, MCap, Volume
    # STOCKS.csv: Ticker, Name, Sector, Industry, Subindustry, Exchange, MCap, Volume
    def read_csv_meta(path, is_etf=False):
        if not os.path.exists(path):
            return pd.DataFrame()
        df = pd.read_csv(path, header=None)
        # Keep extra columns but only map what we need for grouping
        n_cols = min(df.shape[1], 7 if is_etf else 8)
        df = df.iloc[:, :n_cols]

        if is_etf:
            # ETFs.csv format: Ticker, Name, Type, Sector, Industry, MCap, Volume
            col_names = ["Ticker", "Name", "Type", "Sector", "Industry", "MCap", "Volume"][:n_cols]
        else:
            # STOCKS.csv format: Ticker, Name, Sector, Industry, Subindustry, Exchange, MCap, Volume
            col_names = ["Ticker", "Name", "Sector", "Industry", "Subindustry", "Exchange", "MCap", "Volume"][:n_cols]
...
def _compute_chart_for_pair(r):
    ...
    try:
        ...
        if adapt_win and adapt_win != Z_LENGTH:
            adapt_dates, adapt_vals = compute_z_history(a, b, src, window_override=adapt_win, hedge_ratio=hr)
...
def update_active_trades(data, chart_data=None):
    ...
    for t in trades:
        ...
        # Recalculate current Z-score using FIXED mean/std from entry time
        # This ensures Z reversion always tracks actual P&L
        entry_mean = t.get("spreadMean")
        entry_std  = t.get("spreadStd")
        log_r = np.log(data[[a, b]].dropna().clip(lower=1e-10))
        if len(log_r) >= 5:
            hr = t.get("hedgeRatio", 1.0)
            current_spread = float(log_r[a].iloc[-1] - hr * log_r[b].iloc[-1])
            if entry_mean is not None and entry_std and entry_std > 0:
                t["currentZ"] = round((current_spread - entry_mean) / entry_std, 2)
            else:
                # Fallback: use fixed window stats from current data
                ratio = log_r[a].tail(Z_LENGTH) - log_r[b].tail(Z_LENGTH)
                std = ratio.std()
                if std > 0:
        ...
        try:
            ...
            trade_hr = t.get("hedgeRatio", 1.0)
            z_dates, z_vals = compute_z_history(a, b, src, hedge_ratio=trade_hr)
...
if __name__ == "__main__":
    ...
    <div class="controls">
      ...
      <div class="filter-row">
        ...
        <div class="control-group">
          <label>Min WR%</label>
          <button class="step-btn" onclick="stepValue('minWR',-10)">−</button>
          <input type="number" id="minWR" value="0" min="0" max="100" step="10" oninput="applyFilters()" style="width:42px;">
          <button class="step-btn" onclick="stepValue('minWR',10)">+</button>
        </div>
      </div>
    ...
    <div class="modal-overlay" id="chartModal" onclick="closeOnBg(event)">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div id="btMultiTable" style="display:none;padding:12px 16px;background:rgba(13,21,32,0.95);overflow-y:auto;"></div>
        </div>
    ...
    function switchTab(mode) {{
      ...
      document.querySelector(".chart-container").style.display = isBT ? "none" : "";
      ...
      if (isBT && currentChartData) buildBacktestTable(currentChartData);
    }}
    ...
    function buildPriceChart(p) {{
      ...
      // Build per-point background/border colors for coint bars
      const cointBg = cointAligned.map(v => {{
        if (v == null) return "transparent";
        if (v >= 95) return "rgba(34,197,94,0.18)";
        if (v >= 90) return "rgba(245,158,11,0.18)";
        return "rgba(239,68,68,0.18)";
      }});
      const cointBorder = cointAligned.map(v => {{
        if (v == null) return "transparent";
        if (v >= 95) return "rgba(34,197,94,0.5)";
        if (v >= 90) return "rgba(245,158,11,0.5)";
        return "rgba(239,68,68,0.5)";
      }});

      const datasets = [
        {{
          label: "Coint %", data: cointAligned,
          type: "bar", yAxisID: "yCoint", order: 3,
          backgroundColor: cointBg,
          borderColor: cointBorder,
    ...
    function buildBacktestTable(p) {{
      ...
      if (grid.length === 0) {{ mtDiv.innerHTML = '<div style="color:#64748b;text-align:center;padding:20px;">No backtest data</div>'; return; }}
      ...
      let html = '<div style="font-family:var(--mono);font-size:13px;color:#94a3b8;margin-bottom:10px;">'
        + `Win Rate: <span style="color:var(--text);font-weight:600">${{wr}}</span>`
        + ` &nbsp;|&nbsp; ${{wins}}W / ${{losses}}L (${{nt}} trades)`
        + (ar ? ` &nbsp;|&nbsp; Avg ${{ar}} in ${{ad}}` : "")
        + (rv ? ` &nbsp;|&nbsp; ${{rv}}` : "")
        + (md ? ` &middot; ${{md}}` : "")
        + '</div>';
      ...
      html += '<tr style="color:#cbd5e1;border-bottom:2px solid var(--border2);">'
        + '<th style="padding:6px 8px;text-align:left;white-space:nowrap;font-size:12px;">Z \\ Hold</th>';
      holdDays.forEach(hd => {{
        html += `<th style="padding:6px 4px;text-align:center;font-size:11px;">${{hd}}d</th>`;
      }});
      html += '</tr>';

      thresholds.forEach(thr => {{
        html += '<tr>';
        html += `<td style="padding:6px 8px;color:var(--text);font-weight:600;white-space:nowrap;font-size:12px;">${{thr.toFixed(1)}}σ</td>`;
        holdDays.forEach(hd => {{
          const g = lookup[thr + "_" + hd];
          const isBest = thr === bestTh && hd === bestHD;
          const wrVal = g && g.wr != null ? g.wr : null;
          const n = g ? g.n || 0 : 0;
          const wrColor = wrVal == null ? "#64748b" : wrVal >= 70 ? "var(--green)" : wrVal >= 50 ? "var(--amber)" : "var(--red)";
          const bg = isBest ? "rgba(56,189,248,0.15)" : "transparent";
          const star = isBest ? " ★" : "";
    ...
    if WRITE_PLAIN_HTML:
        with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
            f.write(html)
        print(f"{OUTPUT_FILE} created. ({len(top_results)} pairs rendered)")

    if COMPRESS_HTML:
        gz_path = OUTPUT_FILE + ".gz"
        with gzip.open(gz_path, "wb", compresslevel=9) as f:
            f.write(html.encode("utf-8"))
        print(f"{gz_path} created. ({len(top_results)} pairs rendered)")
    ...
    if WRITE_PLAIN_HTML:
        ...
        print(f"\nDone. Decompress {OUTPUT_FILE}.gz to view in a browser.")
